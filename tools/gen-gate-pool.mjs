#!/usr/bin/env node
/* Sample gate-puzzle candidates (js/generate.js: tryGenerateGate) into
   .genwork/gate-pool.json for tools/add-variety-levels.mjs to pick from.
   Deterministic given BASE_SEED, same spirit as gen-hitch-pool.mjs — a
   dedicated pool since gate levels are hand-shaped (sensor+gate placement)
...
import { tryGenerateGate } from '../js/generate.js';
...
for(let seed = BASE_SEED; seed <= BASE_SEED + ATTEMPTS; seed++){
  const rng = mulberry32(hashStr('mg-gate:' + seed));
  const pieces = 6 + (seed % 8);   // 6..13 filler pieces
  const lv = tryGenerateGate(rng, { minOptimal: MIN_OPTIMAL, pieces });
...
writeFileSync(join(WORK, 'gate-pool.json'), JSON.stringify(pool));
const pars = pool.map(l => l.m).sort((a, b) => a - b);
const hist = {};
for(const p of pars) hist[p] = (hist[p] || 0) + 1;
console.log(`\nDone: ${pool.length} unique gate levels in ${((Date.now() - t0) / 1000).toFixed(0)}s`);
