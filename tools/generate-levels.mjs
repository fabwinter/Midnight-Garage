...
const BANDS = [
  ...
  { name: 'Harbor Freight', accent: '#37c8ab', minM: 11, maxM: 19 },
  { name: 'Gridlock',       accent: '#f26fb1', minM: 16, maxM: 60 },
];
...
const hardSeeds = pool.filter(lv => lv.m >= 5).slice(-HARDEN_SEEDS);
hardSeeds.forEach((lv, i) => {
  const rng = mulberry32(hashStr('mg-harden:' + i));
  let hard = harden(lv, rng, HARDEN_STEPS);
  if(hard.m < 12){  // stuck climb — restart with a different mutation stream
    hard = harden(hard, mulberry32(hashStr('mg-harden2:' + i)), HARDEN_STEPS);
  }
  if(hard !== lv && !seen.has(hard.key)){
    seen.add(hard.key);
    pool.push(hard);
  }
...
const chapters = BANDS.map((band, ci) => {
  ...
  distinct.slice(0, PER_CHAPTER).forEach(l => used.add(l.key));
  return distinct.slice(0, PER_CHAPTER);
});
