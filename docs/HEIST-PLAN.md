...
- **Art seam, now with real art**: a car's own `photo` field (top-level,
  ...
  car used at H0. 24 of the 50 job cars now have real art, reusing
  ...
  remaining 26 plus all 4 bounty marks are upload gaps.
  *(Hero art is **sports cars only**. A first pass wired 33 cars by
  reusing whatever body was free, which put a police car in chapter 1 —
  `traffic-sedan-25` is a police livery despite a neutral filename, so a
  grep for "police" missed it and a squad car became a player reward in a
  game whose fail state is the police arriving — plus a rust-weathered
  wreck as "Neon Ghost" and a run of economy sedans and hatchbacks.
  Level 1 hands the player the red sports car the entire premise is built
  on, so following it with a white economy sedan at level 10 reads as a
  demotion rather than a reward. The plain sedans, hatches, SUV, both
  police liveries and the wreck are now traffic-only; the rarity ladder
  runs classic → tuner → muscle → exotic **within** sports cars instead of
  economy→exotic. Cost of the change: 9 fewer cars have art, so the upload
  list grows from 21 to 30. `tools/verify-levels.mjs` now carries a
  denylist so a non-sports body can't be assigned as a hero again.)*
- **Hero light overlay is measured, not assumed** (`js/car-art.data.js`,
  regenerated by `tools/measure-car-art.mjs`). The beam/lamp/taillight
  geometry in `vehicleSVG` was hand-tuned to `classic.webp` and applied
  verbatim to every photo, on the stated assumption that all art shares
  its normalization. Measuring showed it doesn't — nose positions run
  87.9%–99.9% of canvas width against a documented spec of 97% — so the
  glows sat on the lamps for `classic.webp` and drifted onto the bonnet,
  or off the body entirely, everywhere else. Positions now derive from
  each photo's measured extent, with the ratios back-solved so
  `classic.webp` still resolves to its original constants (verified to
  within 0.6 viewBox units). The anchors track the car's front/rear
