...
| Item | Status | Notes |
|---|---|---|
| 0.1 Rebrand + identity | ✅ / 🔶 | Midnight Garage everywhere: title, header, `mg_*` save keys, app icon (`assets/icon.svg`, red car + amber gate glow). 🔶 First-pass trademark/handle check done 2026-08-04 (web search, not a paid clearance search) — no exact app-store-title or USPTO collision, but several real automotive businesses already trade under this name; see STORE-SHIP-PLAN.md P0-9 for the full finding. Still a business decision, not fully closed. |
...
| 0.4 Capacitor shell + native feel | ✅ / 🔶 | `capacitor.config.json`, `package.json` deps, safe-area CSS, theme color. Haptics bridge done (`js/haptics.js`): tick per cell, thud on collision, heavy for trucks, success pattern. `npx cap add ios`/`android`, launch screen assets, `AVAudioSession .ambient` all done 2026-08-04 (turned out not to need Xcode — see STORE-SHIP-PLAN.md P0-3/P0-4). 🔶 Never compiled — no Xcode/Android SDK in this environment, see docs/CI-SETUP.md. |
