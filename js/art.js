...
import { CAR_ART_GEOMETRY } from './car-art.data.js';
...
export function vehicleSVG(idx, len, dir, isHero, opts = {}){
  ...
  /* Hero (classic photo car): geometry measured from the normalized
     ...
     Drawn for EVERY hero, whatever its body art: classic.webp, a bespoke
     `skin.photo` render, or the recolored SEDAN_PHOTOS[0] fallback.

     These used to be hardcoded constants, on the stated assumption that
     every photo shares classic.webp's normalization (front at the right
     end, ~97% of the canvas length) so "the anchors land close enough on
     each." Measuring the shipped art showed that assumption doesn't hold:
     nose positions actually run 87.9%–99.9% of canvas width. The glows sat
     on the lamps for classic.webp and drifted onto the bonnet — or clean
     off the body — for the rest. So the positions below are now derived
     from each photo's measured extent (js/car-art.data.js, regenerated by
     tools/measure-car-art.mjs), with the ratios chosen so classic.webp
     still resolves to exactly the constants it was hand-tuned to.

     The anchors track the car's front/rear CORNERS, not its baked lamps:
     locating the lamps themselves was tried and abandoned (both absolute
     brightness and a per-car brightness percentile pick up the whole
     panel on white and cream bodies, where the paint is as bright as the
     lights). Corner-anchoring isn't pixel-perfect on every lamp, but it
     keeps the glow on the vehicle, which is the defect that actually
     reads. A photo with no measured entry — e.g. an admin library upload —
     falls back to classic.webp's geometry, i.e. the old behaviour.

     A job-car pass briefly withheld this for fallback heroes on the theory
     that the traffic photo wasn't shot to classic.webp's layout; that shipped
  ...
  const heroPhotoPath = !skin ? CLASSIC_CAR_IMG : (skin.photo || SEDAN_PHOTOS[0].img);
  const geo = CAR_ART_GEOMETRY[heroPhotoPath] ?? CAR_ART_GEOMETRY[CLASSIC_CAR_IMG];
  // Measured percentages -> this piece's viewBox units.
  const nx = geo.nose / 100 * L, tx = geo.tail / 100 * L;
  const fT = geo.fTop / 100 * H, fB = geo.fBot / 100 * H, fSpan = fB - fT;
  const rT = geo.rTop / 100 * H, rB = geo.rBot / 100 * H, rSpan = rB - rT;
  // Ratios back-solved from classic.webp so it renders bit-identically to
  // the previous hardcoded values; every other car now gets the same
  // relationship to ITS own nose/tail instead of inheriting classic's.
  const lampX = nx - 0.096 * L, coreX = nx - 0.061 * L, beamX = nx - 0.081 * L, beamLipX = nx - 0.021 * L;
  const lampY1 = fT + 0.104 * fSpan, lampY2 = fB - 0.104 * fSpan;
  const coreY1 = fT + 0.154 * fSpan, coreY2 = fB - 0.154 * fSpan;
  const tailX = tx + 0.019 * L, tailGlowX = tx + 0.004 * L;
  const tailY1 = rT + 0.083 * rSpan, tailY2 = rB - 0.083 * rSpan;
  const far = L + 185;
  const photoHeroExtra = (isHero && opts.headlights !== false) ? `
    <path d="M ${beamX} ${lampY1 - 4} L ${far} ${lampY1 - 26} L ${far} ${lampY1 + 28} L ${beamLipX} ${lampY1 + 12} Z" fill="url(#${gid}beam2)" filter="url(#${gid}bblur)"/>
    <path d="M ${beamX} ${lampY2 + 4} L ${far} ${lampY2 + 26} L ${far} ${lampY2 - 28} L ${beamLipX} ${lampY2 - 12} Z" fill="url(#${gid}beam2)" filter="url(#${gid}bblur)"/>
    <ellipse cx="${lampX}" cy="${lampY1}" rx="15" ry="4.5" transform="rotate(24 ${lampX} ${lampY1})" fill="#fff3c2" opacity=".55" filter="url(#${gid}bblur)"/>
    <ellipse cx="${lampX}" cy="${lampY2}" rx="15" ry="4.5" transform="rotate(-24 ${lampX} ${lampY2})" fill="#fff3c2" opacity=".55" filter="url(#${gid}bblur)"/>
    <circle cx="${coreX}" cy="${coreY1}" r="3" fill="#fffbe8"/>
    <circle cx="${coreX}" cy="${coreY2}" r="3" fill="#fffbe8"/>
    <ellipse cx="${tailX}" cy="${tailY1}" rx="5" ry="7" fill="#ff4a3a" opacity=".55" filter="url(#${soft})"/>
    <ellipse cx="${tailX}" cy="${tailY2}" rx="5" ry="7" fill="#ff4a3a" opacity=".55" filter="url(#${soft})"/>
    <ellipse cx="${tailGlowX}" cy="${(rT + rB) / 2}" rx="5" ry="${rSpan * 0.368}" fill="#ff3b2e" opacity=".24" filter="url(#${soft})"/>` : '';
